...
struct OperationalRepresentativeDays <: OperationCostModel
    ...
    OperationalRepresentativeDays(representative_series, series_weights) = length(series_weights) == length(representative_series) ? new(representative_series, series_weights) : error("Length of weights and number of representative days are different")    
